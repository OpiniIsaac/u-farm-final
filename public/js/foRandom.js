function generateUniqueNumber() {
    ...
    let uniqueNumber = "UF-" + randomNumber.toString().padStart(3, "0"); // Pad the number with leading zeros if necessary
    while (!unregex.test(uniqueNumber)) { // Check if the number already exists
      randomNumber = Math.floor(Math.random() * 1000);
      uniqueNumber = "UF-" + randomNumber.toString().padStart(3, "0");
    }
    return uniqueNumber;
  }
